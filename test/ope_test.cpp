...
TEST_F(OPETest, D0CInt) {
  ...
  EXPECT_NEAR(OPE::D0CIntFO(2.6, Weight(6), 3.1570893124000001, 0.29, c, 5), 0.28350497970822786, 1e-14);
  EXPECT_NEAR(OPE::D0CIntFO(3.0, Weight(1), pow(1.77686, 2), 0.3156, c, 5), 1.8062025328897611, 1e-14);
  EXPECT_NEAR(OPE::D0CIntFO(2.0, Weight(3), pow(1.77686, 2), 0.3156, c, 5), 0.22393804167894238, 1e-14);

  // CIPT
  EXPECT_NEAR(OPE::D0CIntCI(2.6, Weight(1), 3.1570893124000001, 0.31927, c, 1), 1.7203914169060759, 1e-13);
  EXPECT_NEAR(OPE::D0CIntCI(2.6, Weight(1), 3.1570893124000001, 0.31927, c, 2), 1.7643779920232532, 1e-13);
  EXPECT_NEAR(OPE::D0CIntCI(2.6, Weight(1), 3.1570893124000001, 0.31927, c, 5), 1.8005358682689818, 1e-13);
}
...
TEST_F(OPETest, D68) {
  // D68(s, rho, c8)
  EXPECT_NEAR(OPE::D6(3.0, -0.1894).real()+OPE::D8(3.0, 0.16315).real(), -1.2987654320987656e-4, 1e-15);
