...
TEST_F(AdlerFunctionTest, D0) {
  ...
  EXPECT_NEAR(adler->D0(s, mu2, adler->kAlphaTau).real(), 2.6878635987293748e-2, 1e-13);
  EXPECT_NEAR(adler->D0(s, mu2, adler->kAlphaTau).imag(), 1.5454361164073294e-3, 1e-13);

  s = complex<double>(7.0, 2.0);
  mu2 = complex<double>(1.5, 2.2);
  EXPECT_NEAR(adler->D0(s, mu2, adler->kAlphaTau).real(), 2.6732096654905575e-2, 1e-13);
  EXPECT_NEAR(adler->D0(s, mu2, adler->kAlphaTau).imag(), -1.0995430321549232e-3, 1e-13);
}

TEST_F(AdlerFunctionTest, CIntD0) {
  double s0 = 3.;
  EXPECT_NEAR(adler->d0ContourIntegral(s0, wD00, adler->kAlphaTau), 1.8129891021138491, 1e-13);
  // EXPECT_NEAR(adler->contourIntegral(s0, wD00).imag(), 0., 1e-13);

  s0 = 2.6;
  EXPECT_NEAR(adler->d0ContourIntegral(s0, wD00, adler->kAlphaTau), 1.8396044152966260, 1e-13);
  // EXPECT_NEAR(adler->contourIntegral(s0, wD00).imag(), 0., 1e-13);
}
