...
void print_state(unsigned iter, gsl_multiroot_fdfsolver *s) {
  printf (
    "iter = %3u x = % .10f % .10f "
    "f(x) = % .3e % .3e\n",
    iter,
...
int AlphaS::alpha_f(const gsl_vector *x, void *params, gsl_vector *f) {
...
int AlphaS::alpha_df(const gsl_vector *x, void *params, gsl_matrix *J) {
  (void)params;
  const double ap1 = gsl_vector_get (x, 0);
  const double ap2 = gsl_vector_get (x, 1);
...
cmplx AlphaS::runAlpha(const cmplx &mup, const cmplx &muq, const cmplx &aq) {
